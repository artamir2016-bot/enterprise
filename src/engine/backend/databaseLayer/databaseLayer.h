...
struct ibDialectDictionary
{
	...
	// columns can still overflow if one is wide. Firebird's index key is bounded at roughly a QUARTER
	// of the page size, and a string column is declared in CHARACTERS but indexed in BYTES — a
	// VARCHAR(255) in a UTF8 database is 1020 bytes, so two of them pass the ceiling on a key of two
	// fields (well under m_maxIndexSegments). Firebird then refuses CREATE INDEX with "key size exceeds
	// implementation restriction", which — like the segment overflow — takes the whole restructuring
	// down and names a table that is not the cause. Declared for the same reason as the segment count:
	// the identity moves into one hashed field BEFORE any DDL is emitted (ibDeclareDerivedKey).
